...
#######################################################################
from proj.code.se3_control import SE3Control
from proj.code.world_traj import WorldTraj
#######################################################################
...
file = Path(inspect.getsourcefile(lambda:0)).parent.resolve() / 'proj' / 'util' / filename
if not (Path('.') / 'proj' / 'img').is_dir():
    (Path('.') / 'proj' / 'img').mkdir()
...
x_est = est_state['x']
ax = axes[0]
ax.plot(sim_time, x_est[:,0], 'r--', sim_time, x_est[:,1], 'g--', sim_time, x_est[:,2], 'b--', linewidth=2.5)
ax.legend(('x', 'y', 'z', 'xdes', 'ydes', 'zdes', 'xest', 'yest', 'zest'), loc='lower right')
...
plt.tight_layout()
plt.savefig(f"proj/img/pos_vel_window.png", dpi=600)
...
ani = animate(sim_time, state['x'], R, world=world, filename=f"proj/img/{filename.split('.')[0]}.mp4")
